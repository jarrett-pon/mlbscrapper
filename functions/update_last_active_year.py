...
SET_LAST_ACTIVE_YEAR = ("UPDATE players SET last_active_year = %s WHERE id = %s")
...
def update_last_active_year():
    ...
    for player in database_players:
        ...
        if game_types_count == "0":
            years_played = []
        elif game_types_count == "1" and game_types_results["row"]["game_type"] == "R":
            years_played = [game_types_results["row"]["season"]]
        elif game_types_count != "1":
            years_played = [game_type["season"] for game_type in game_types_results["row"] if game_type["game_type"] == "R" ]
        else:
            years_played = []
        print(years_played)
    ...
    print("Players with last active years updated", all_last_active_year_players)
    db = Database()
