...
TEAM_INFO_EXT = (
    ...
    "?season=2017&game_date='2017/10/01'&sit_code='h0'&league_id=103"
    "&league_id=104"
)
# MySQL query constants
GET_TEAMS = (
    "SELECT mlb_id FROM teams"
)
ADD_TEAMS = (
    "INSERT INTO teams (name, short_name, abbreviation, mlb_id)"
    " VALUES (%s, %s, %s, %s)"
)
...
# Response structure
mlb_data = (
    response["historical_standings_schedule_date"]["standings_all_date_rptr"]
    ["standings_all_date"]
)
...
for league in mlb_data:
    ...
    for team in teams:
        team_data = (
            team["team_full"], team["team_short"],
            team["team_abbrev"], team["team_id"]
        )
...
all_team_data = [team
                 for team
                 in all_team_data if int(team[3]) not in database_teams]
